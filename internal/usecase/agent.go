...
func (u *AgentUsecase) CreateNewAgent(userId, name, description, aiModel, aiProvider string, agentType entity.AgentType, credit_per_1k int, status entity.AgentStatus) (*entity.Agent, error) {
	if userId == "" || name == "" {
		return nil, appErrors.NewValidationError("User ID and name are required")
	}

	agent, err := u.Agent.CreateAgent(userId, name, description, aiModel, aiProvider, agentType, credit_per_1k, status)
	if err != nil {
		return nil, err
	}
	return agent, nil
